...
void PropagateBundle::start()
{
    ...
    quint64 currentBundleSize = 0;
    quint64 currentRequestsNumber = 0;
    while (!_itemsToSync.isEmpty()){
        SyncFileItemPtr item = _itemsToSync.takeFirst();
        currentBundleSize += item->_size;
        currentRequestsNumber++;
        //Break the loop if bundle cannot fit more files in the bundle.
        if ((currentBundleSize >= chunkSize()) || (currentRequestsNumber > checkBundledRequestsLimits())){
...
void PropagateBundle::append(const SyncFileItemPtr &bundledFile){
    ...
    _itemsToSync.append(bundledFile);
}

bool PropagateBundle::empty(){
    return _itemsToSync.empty();
}
...
void PropagateBundle::slotMultipartFinished()
{
    ...
    if (_itemsToSync.isEmpty()){
        done(SyncFileItem::NoStatus);
    } else {
        start();
    }
}
...
// This function is used whenever there is an error occuring and we need to raise status for more files in the bundle
